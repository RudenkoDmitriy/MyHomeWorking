...
NodeOfTree *createArithmTree(NodeOfTree *root, std::string line, int &position)
{
	if (sizeof(line) > 0)
	{
		...
		{	
			...
			position++;
			root->left = createArithmTree(root->left, line, position);
			root->right = createArithmTree(root->right, line, position);
		}
...
int countingArithmTree(NodeOfTree *root, bool &test)
{
	if (root->key == '*')
	{
		return countingArithmTree(root->left, test) * countingArithmTree(root->right, test);
	}
	if (root->key == '/')
	{
		if (countingArithmTree(root->right, test) == 0)
		{
...
void printArithmTree(NodeOfTree *root)
{
	...
	{
		printf("( %c ", root->key);
		printArithmTree(root->left);
		printArithmTree(root->right);
		printf(") ");
	}
	else
	{
